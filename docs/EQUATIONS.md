...
The R amplitude factor preserves integrated power under reduction before aperture losses. P is amplitude, not intensity transmission. The physical pupil field includes `1 / (i λ f1)`; Fourier quadrature includes mask pixel area. See [lithography_coherent_fields.m](../src/optics/lithography_coherent_fields.m).
...
Two Bluestein transforms evaluate it on explicit physical axes. See [lithography_lct.m](../src/optics/lithography_lct.m). Near nominal image focus, same-grid Fresnel propagation uses `H_F = exp[-iπ λ d (νx² + νy²)]` with bounded padding and anti-aliasing, or the corresponding convolution kernel. See [lithography_fresnel_same.m](../src/optics/lithography_fresnel_same.m).
